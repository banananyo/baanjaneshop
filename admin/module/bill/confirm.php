    ...
    <div class="container main-container" ng-controller="manage as controller">
        ...
        <div class="col-md-8 content">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <a class="btn btn-default" href="index.php?module=bill&mode=confirm&get=ALL">
                        ทั้งหมด
                    </a>
                    <a class="btn btn-default" href="index.php?module=bill&mode=confirm&get=WFC">
                        รอการยืนยัน
                    </a>
                    <hr>
                    <table class="table table-condensed table-hover">
                        <tr>
                            ...
                            <td class="col-md-5">หมายเลขบิล</td>
                        ...
                        $sql_count = $conn->query("SELECT COUNT(*) FROM `payment_confirm` WHERE $filter");
                        $count = $sql_count->fetch_row();
                        $count = $count[0];

                        $sql = "SELECT * FROM `payment_confirm` WHERE $filter";
                    ...
                    ?>
                            <tr>
                                ...
                                <td>
                                    <?php echo getBillID($conn,$value['orders_ref']); ?>
                                </td>
                                <td>
                                    <a href="index.php?module=bill&mode=view_bill&id=<?php echo $value['id'] ?>" class="btn btn-default">ดูรายละเอียด</a>
                                    <a href="javascript:;" ng-click="controller.approve('<?php echo $value['id'] ?>')"  class="btn btn-success <?php echo ($value['status'] == "WFC" ? "":"is-hidden") ?>">อนุมัติ</a> 
                                    <a href="javascript:;" ng-click="controller.rejected('<?php echo $value['id'] ?>')" class="btn btn-danger <?php echo ($value['status'] == "WFC" ? "":"is-hidden") ?>">ไม่อนุมัติ</a> 
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            <li>
                            <a class="<?php echo 1 == $page ? 'is-hidden':"" ?>" href="index.php?module=confirm&mode=manage&page=<?php echo isset($_GET['page']) ? $_GET['page']-1:2; ?>" aria-label="Previous">
                            ...
                            for($i=1;$i<=ceil($count / $limit);$i++) {
                                ...
                                echo " <li class='".($page == $i ? 'active':'')."'><a href=\"index.php?module=confirm&mode=manage&page=$i$textLimit\">$i</a></li>";
                            } ?>
                           
                            <li>
                            <a class="<?php echo ceil($count / $limit) == $page ? 'is-hidden':"" ?>" href="index.php?module=confirm&mode=manage&page=<?php echo isset($_GET['page']) ? $_GET['page']+1:2; ?>" aria-label="Next">
