...
<?php
    ...
    $id = $_GET['id'];
    $row = Connect::conn()->get("promotion","*",['id' => $_GET['id']]);
...
<form action="" method="post"  enctype="multipart/form-data">
  <div class="box">
    <div class="box-header with-border">
      <h3 class="box-title">แก้ไขโปรโมชัน</h3>
    </div>
    <div class="box-body">
     <?php 
        if(isset($_POST['button'])  && $_POST['button'] == "confirm") {
            $target_dir = "uploads/";
                            ...
                            if($_FILES['image']["name"] != "") {
                                ...
                                $image =  basename($newfilename);
                                Connect::conn()->update("promotion", [
                                    'description' => $description,
                                    'src' => $image,
                                ],['id' => $_GET['id']]);
                                if(Connect::conn()->has("promotion", [
                                    'description' => $description,
                                    'src' => $image,
                                    ])) {
                                    $resultTxt = "สำเร็จ";
                        ...
                        <div class="is-center">
                            <h4>แก้ไขโปรโมชัน<?php echo $resultTxt; ?> </h4>
